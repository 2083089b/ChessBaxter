...
def hog(img):
    ...
    hist = np.concatenate((M, hist), axis=0)
    #
    # hist = normalize(hist[:, np.newaxis], axis=0).ravel()
    return hist
...
def chess_train(out_dir_desc, train_classes, train_mod="LR_l1"):
    ...
    if train_mod == "LR":
        classification_model = LogisticRegression(C = params_opt['C'], penalty = params_opt['penalty'], solver = params_opt['solver'])
    elif train_mod == "SVM":
        classification_model = SVC(C = params_opt['C'], kernel = params_opt['kernel'], probability=True, random_state=np.random.RandomState(0))
    else:
        raise Exception("Unknown classification model")
...
def grid_search(X, y, tuned_parameters, train_mod):
    ...
    for score in scores:
        ...
        if train_mod == "SVM":
            clf = GridSearchCV(SVC(C=1, probability=True, random_state=np.random.RandomState(0)), tuned_parameters, cv=5, scoring='%s_macro' % score)
        if train_mod == "LR":
